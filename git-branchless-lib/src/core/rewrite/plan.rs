...
use crate::core::dag::{CommitSet, Dag, sorted_commit_set, union_all};
...
impl RebaseCommand {
    /// Convert the command to one or more lines in Git's interactive rebase
    /// plan format.
    pub fn to_rebase_commands(&self) -> Vec<String> {
        match self {
            RebaseCommand::CreateLabel { label_name } => vec![format!("label {label_name}")],
            RebaseCommand::Reset { target } => vec![format!("reset {target}")],
            RebaseCommand::Pick {
                original_commit_oid: _,
                commits_to_apply_oids,
            } => match commits_to_apply_oids.as_slice() {
                [] => Vec::new(),
                [commit_oid] => vec![format!("pick {commit_oid}")],
                [first_commit_oid, rest @ ..] => {
                    let mut commands = vec![format!("pick {first_commit_oid}")];
                    commands.extend(rest.iter().map(|commit_oid| format!("fixup {commit_oid}")));
                    commands
                }
            },
            RebaseCommand::Merge {
            ...
            RebaseCommand::Replace {
                ...
                // triggered.
                vec![format!(
                    "exec git show -s --format=%B {replacement_commit_oid} | git commit-tree {parents} {replacement_commit_oid}^{{tree}}"
                )]
            }
            RebaseCommand::Break => vec!["break".to_string()],
            RebaseCommand::RegisterExtraPostRewriteHook => {
                vec!["exec git branchless hook-register-extra-post-rewrite-hook".to_string()]
            }
            RebaseCommand::DetectEmptyCommit { commit_oid } => {
                vec![format!(
                    "exec git branchless hook-detect-empty-commit {commit_oid}"
                )]
            }
            RebaseCommand::SkipUpstreamAppliedCommit { commit_oid } => {
                vec![format!(
                    "exec git branchless hook-skip-upstream-applied-commit {commit_oid}"
                )]
            }
        }
    }
        ...
        for constraint in roots {
            ...
            let (parent_oids, child_oid) = match constraint {
                ...
                Constraint::MoveChildren {
                    ...
                    "BUG: Invalid constraint encountered while preparing rebase plan.\nThis should be unreachable."
                ),
            };
